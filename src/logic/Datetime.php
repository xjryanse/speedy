...
class Datetime
{
    ...
    public static function scopeTimeStr($startTime, $endTime){
        ...
        $startStr       = $startTime ? date('Y年m月d日',strtotime($startTime)) : '';
        $startTimeStr   = $startTime ? date('H:i',strtotime($startTime)) : '';
        ...
        if($startDay == $endDay){
            ...
            // 20250820
            if($endStr == '23:59' && $startTimeStr = '00:00'){
                $startTimeStr = '';
                $endStr = '';
            }
        } else {
        ...
        return $endStr ? $startStr.$startTimeStr.'至'.$endStr : $startStr.$startTimeStr;
    }
